...
class LiveCodingView(ScrollableContainer):
    ...
    def compose(self) -> ComposeResult:
        yield Label("🎵 Live Coding - Python REPL", id="live-coding-header")
        yield TextArea(
            self.get_default_code(),
            id="code-input",
            language="python"
        )
        yield Static(self.render_output(), id="output-display")
        yield Label(
            "[Ctrl+R] Run Code | [Ctrl+P] Play Audio | [Ctrl+S] Save Audio | [Tab] Next View",
            id="live-coding-help"
        )
    
    def get_default_code(self) -> str:
        """Get default code for the editor."""
        return """# Algorythm Live Coding
...
Examples:
  ...
  • result = audio  # Store for playback
"""
        
        output = ["[Output Console]\n"]
        # Show last 20 lines of output
        for line in self.output_history[-20:]:
            output.append(line)
        return "\n".join(output)
    ...
    async def run_code(self):
        ...
        try:
            ...
            self.output_history.append(f"✗ Error getting code: {e}")
            self.refresh_output()
            return
        
        self.output_history.append("=" * 60)
        self.output_history.append(">>> Running code...")
        self.refresh_output()
        ...
        try:
            ...
            # Import algorythm modules into namespace
            exec_globals = {
                '__name__': '__main__',
                'project': self.project,
                'np': np,
            ...
            # Restore stdout before processing results
            ...
            if 'result' in exec_globals and exec_globals['result'] is not None:
                ...
                if isinstance(result, np.ndarray):
                    ...
                    self.output_history.append(f"✓ Audio generated: {len(self.current_audio)} samples")
                    self.output_history.append("  Use Ctrl+P to play, Ctrl+S to save")
                else:
                    self.output_history.append(f"✓ Code executed successfully")
                    self.output_history.append(f"  Result type: {type(result).__name__}")
            else:
                self.output_history.append("✓ Code executed successfully")
            
        except Exception as e:
            import traceback
            import sys
            sys.stdout = old_stdout  # Make sure stdout is restored
            self.output_history.append(f"✗ Error: {str(e)}")
            tb_lines = traceback.format_exc().split('\n')
            for line in tb_lines:
                if line.strip():
                    self.output_history.append(f"  {line}")
    ...
    def play_audio(self):
        ...
        if self.current_audio is None:
            self.output_history.append("⚠ No audio to play. Run code first (Ctrl+R)")
            self.refresh_output()
            return
        ...
        try:
            ...
            player = AudioPlayer()
            self.output_history.append("▶ Playing audio...")
            self.refresh_output()
            # Note: blocking playback in async context - should be improved
            player.play(self.current_audio, blocking=False)
            self.output_history.append("✓ Playback started")
        except ImportError:
            self.output_history.append("⚠ PyAudio not installed. Install with: pip install pyaudio")
        except Exception as e:
            self.output_history.append(f"✗ Playback error: {e}")
    ...
    def save_audio(self):
        ...
        if self.current_audio is None:
            self.output_history.append("⚠ No audio to save. Run code first (Ctrl+R)")
            self.refresh_output()
            return
        ...
        try:
            ...
            import time
            filename = f"algorythm_live_{int(time.time())}.wav"
            exporter = Exporter()
            exporter.export(self.current_audio, filename)
            self.output_history.append(f"✓ Saved to: {filename}")
        except Exception as e:
            self.output_history.append(f"✗ Save error: {e}")
    ...
    def refresh_output(self):
        ...
        output_widget.update(self.render_output())
...
class TerminalDAW(App):
    ...
    def __init__(self):
        ...
        self.current_view_index = 4  # Start with Live Coding view (index 4)
        self.views = []
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="view-container"):
            ...
            # Start with live coding view visible (index 4)
