...
For more information, visit: https://github.com/KiyomiLovesOreos/algorythm
        """,
    )
    ...
    visualize_parser = subparsers.add_parser(
        "visualize", help="Create visualization video from audio file"
    )
    visualize_parser.add_argument("input_file", help="Input audio file (MP3, WAV)")
    visualize_parser.add_argument(
        "-o", "--output", help="Output video file (default: input_video.mp4)"
    )
    visualize_parser.add_argument(
        "-v",
        "--visualizer",
    ...
    visualize_parser.add_argument(
        "-w", "--width", type=int, default=1920, help="Video width (default: 1920)"
    )
    visualize_parser.add_argument(
        "--height", type=int, default=1080, help="Video height (default: 1080)"
    )
    visualize_parser.add_argument(
        "--fps", type=int, default=30, help="Video FPS (default: 30)"
    )
    visualize_parser.add_argument(
        "--offset", type=float, default=0.0, help="Start offset in seconds (default: 0)"
    )
    visualize_parser.add_argument(
        "--duration",
        type=float,
    ...
    formats_parser = subparsers.add_parser(
        "formats", help="List supported audio formats"
    )

    # Generate music command
    generate_parser = subparsers.add_parser(
        "generate", help="Generate music from command line"
    )
    generate_parser.add_argument(
        "-i", "--instrument", default="pluck", help="Instrument preset (default: pluck)"
    )
    generate_parser.add_argument(
        "-s",
        "--scale",
    ...
    generate_parser.add_argument(
        "-t", "--tempo", type=int, default=120, help="Tempo in BPM (default: 120)"
    )
    generate_parser.add_argument(
        "-r",
        "--repeat",
    ...
    generate_parser.add_argument(
        "-o", "--output", default="output.wav", help="Output file (default: output.wav)"
    )
    generate_parser.add_argument(
        "--effects", help="Effects chain (comma-separated, e.g., reverb,delay,chorus)"
    )

    # List presets command
    presets_parser = subparsers.add_parser(
        "presets", help="List available instrument presets"
    )

    # List effects command
    effects_parser = subparsers.add_parser(
        "effects", help="List available audio effects"
    )

    # Apply effects command
    apply_fx_parser = subparsers.add_parser(
        "apply-fx", help="Apply effects to audio file"
    )
    apply_fx_parser.add_argument("input_file", help="Input audio file")
    ...
    apply_fx_parser.add_argument(
        "-o", "--output", help="Output file (default: input_with_fx.wav)"
    )
    apply_fx_parser.add_argument("--params", help="Effect parameters in JSON format")

    # Quick generate command
    quick_parser = subparsers.add_parser(
        "quick", help="Quick music generation with presets"
    )
...
def run_example(example_name):
    ...
    if example_name == "basic":
        ...
        from algorythm.synth import ADSR, Filter, Synth
        ...
        from algorythm.sequence import Arpeggiator, Motif, Scale
        ...
        bass_motif = Motif.from_intervals(
            [0, 0, 0, 0], scale=Scale.major("A", octave=2)
        )
        composition.add_track("Bass", bass_synth).repeat_motif(bass_motif, bars=4)

        lead_synth = SynthPresets.pluck()
        lead_motif = Motif.from_intervals(
            [0, 2, 4, 7, 9, 7, 4, 2], scale=Scale.major("A", octave=4)
        )
        arpeggiator = Arpeggiator(pattern="up-down", octaves=2)
        arpeggiated = arpeggiator.arpeggiate(lead_motif)

        composition.add_track("Lead", lead_synth).repeat_motif(
            arpeggiated, bars=2
        ).add_fx(Delay(delay_time=0.25, feedback=0.3, mix=0.3)).add_fx(Reverb(mix=0.2))

        pad_synth = SynthPresets.warm_pad()
        pad_motif = Motif.from_intervals([0, 4, 7], scale=Scale.major("A", octave=3))
        composition.add_track("Pad", pad_synth).repeat_motif(pad_motif, bars=4).add_fx(
            Reverb(mix=0.5)
        )
...
def visualize_audio(args):
    ...
    from algorythm.audio_loader import AudioFile, visualize_audio_file
    ...
    try:
        ...
        audio = AudioFile(str(input_path), offset=args.offset, duration=args.duration)
    ...
    if args.visualizer == "waveform":
        ...
        viz = CircularVisualizer(
            sample_rate=audio.sample_rate, num_bars=args.bars, debug=args.debug
        )
    ...
    try:
        ...
        renderer.render_frames(audio.signal, viz, output_path=output_path)
    except Exception as e:
...
def generate_music(args):
    ...
    comp.add_track("Main", instrument)
    track = comp.tracks["Main"]

    # Repeat motif
    current_time = 0.0
    motif_duration = sum(durations)
    for _ in range(args.repeat):
        track.add_motif(motif, start_time=current_time, tempo=args.tempo)
    ...
    # No need to add track again since comp.add_track() already did it
    ...
    # Export
    exporter = Exporter()
    exporter.export(audio, args.output, sample_rate=comp.sample_rate)
...
def apply_effects(args):
    ...
    exporter = Exporter()
    exporter.export(processed, output_file, sample_rate)
...
def quick_generate(args):
    ...
    if args.style == "ambient":
        ...
        motif = Motif(intervals=[0, 2, 4], durations=[8.0, 8.0, 8.0])
        track = Track("Pad")
        ...
        bass_motif = Motif(
            intervals=[scale.get_frequency(-12)] * 8, durations=[0.5] * 8
        )
        ...
        drum_motif = Motif(
            intervals=[scale.get_frequency(-12)] * 16, durations=[0.25] * 16
        )
        ...
        pad_motif = Motif(intervals=[0, 4, 7], durations=[8.0, 8.0, 8.0])
        pad_track = Track("Pad")
        ...
        lead_motif = Motif(
            intervals=scale.ascending(8) + scale.descending(8), durations=[0.25] * 16
        )
        ...
        bass_motif = Motif(
            intervals=[scale.get_frequency(-12)] * 4, durations=[1.0] * 4
        )
        bass_track = Track("Bass")
        bass_track.add_motif(bass_motif, instrument=bass)

        pluck_motif = Motif(intervals=[0, 2, 4, 5], durations=[0.5] * 4)
        ...
        string_motif = Motif(
            intervals=[scale.get_frequency(i) for i in [0, 3, 7, 10, 7, 3]],
            durations=[1.5] * 6,
        )
        ...
        wind_motif = Motif(
            intervals=[scale.get_frequency(i) for i in [5, 7, 9, 12]],
            durations=[2.0] * 4,
        )
    ...
    # Export
    exporter = Exporter()
    exporter.export(audio, args.output, sample_rate=44100)
...
def create_effect_by_name(name, **kwargs):
    ...
    effects = {
        "reverb": lambda: Reverb(
            room_size=kwargs.get("room_size", 0.7),
            wet_level=kwargs.get("wet_level", 0.3),
        ),
        "delay": lambda: Delay(
            delay_time=kwargs.get("delay_time", 0.5),
        ...
        "tremolo": lambda: Tremolo(
            rate=kwargs.get("rate", 5.0), depth=kwargs.get("depth", 0.5)
        ),
        ...
        "compressor": lambda: Compressor(
            threshold=kwargs.get("threshold", -20.0), ratio=kwargs.get("ratio", 4.0)
        ),
        "limiter": lambda: Limiter(threshold=kwargs.get("threshold", -1.0)),
        "gate": lambda: Gate(threshold=kwargs.get("threshold", -40.0)),
        "ringmod": lambda: RingModulator(
            carrier_freq=kwargs.get("carrier_freq", 440.0)
        ),
