...
def launch_studio(args):
    """Launch the Terminal DAW."""
    try:
        from algorythm.terminal_daw import TerminalDAW
        print("\n" + "="*60)
        ...
        if args.project_file:
            ...
            if os.path.exists(args.project_file):
                app.load_project(args.project_file)
            else:
                print(f"Warning: Project file '{args.project_file}' not found. Starting new project.")
        app.run()
    except ImportError as e:
        print(f"Error: Could not launch Terminal DAW. Missing dependencies?")
        print(f"Details: {e}")
        print("\nTry installing with: pip install textual")
        sys.exit(1)
    except Exception as e:
        print(f"Error launching Terminal DAW: {e}")
        sys.exit(1)
